...
export function first(input) {
    ...
    movements.forEach((mov) => {
        const direction = mov[0];
        const amount = parseInt(mov.substring(1)); 
        pointer = direction === 'R' ? (pointer + amount) % 100 : (pointer - amount + 10000) % 100;
        zeroCount += pointer === 0 ? 1 : 0;
    });
...
export function second(input) {
    ...
    let dialPointer = 50;
    movements.forEach(mov => {
        // Get the direction and amount of the movement
        const direction = mov[0];
        const amount = parseInt(mov.substring(1)); 
        // Get moves till zero (first count)
        const distanceToZero = direction === 'R' ? (100 - dialPointer) % 100 : dialPointer;
        // How many zero crosses after first one?
        const crosses = Math.floor((amount - distanceToZero - 1) / 100);
        // Add to zeros counter
        zeroCounts += (1 + crosses);
        // Move pointer
        dialPointer = direction === 'R' ? (dialPointer + amount) % 100 : (dialPointer - amount + 10000) % 100;

        // console.log(`Amount-${direction}: `, amount,"From:", startPointer, "- To:", dialPointer);
